...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if norm_type == "ada_norm":
            self.norm1 = AdaLayerNorm(dim, num_embeds_ada_norm, chunk_dim=1)
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            if norm_type == "ada_norm":
                self.norm2 = AdaLayerNorm(
                    dim, num_embeds_ada_norm, chunk_dim=1)
            elif norm_type == "ada_norm_continuous":
    ...
    def forward(
        ...
        if (self.num_embeds_ada_norm is None):
            assert temb is not None, "temb must be provided"

        if self.norm_type == "ada_norm":
            if (temb is not None):
                norm_hidden_states = self.norm1(
                    hidden_states, timestep=None, temb=temb)
            else:
